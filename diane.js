...
var $author$project$Main$init = function (prog) {
	return _Utils_Tuple2(
		$author$project$Main$initModel(prog),
		...
		while (true) {
			switch (_v0.a.$) {
				...
				case 'Call':
					var id = _v0.a.a;
					var s = _v0.b;
					var _v68 = A2($author$project$Diane$lookup, id, config.env);
					if (_v68.$ === 'Nothing') {
						return $elm$core$Result$Err($author$project$Diane$UnknownVariable);
					} else {
						if (_v68.a.$ === 'Subroutine') {
							var body = _v68.a.a;
				...
				case 'Lookup':
					var id = _v0.a.a;
					var s = _v0.b;
					var _v69 = A2($author$project$Diane$lookup, id, config.env);
					if (_v69.$ === 'Nothing') {
						return $elm$core$Result$Err($author$project$Diane$UnknownVariable);
					} else {
						if (_v69.a.$ === 'Number') {
...
var $author$project$Main$eval = function (model) {
	var go = function (m) {
		...
		while (true) {
			if (m.going) {
				var $temp$m = A2($author$project$Main$step, false, m);
				m = $temp$m;
				continue go;
			} else {
				return m;
...
var $author$project$Main$update = F2(
	function (msg, m) {
		...
		switch (msg.$) {
			case 'Step':
				return mk(
					A2($author$project$Main$step, true, m));
			case 'Eval':
				return mk(
					$author$project$Main$eval(m));
			case 'Undo':
				return mk(
					$author$project$Main$undo(m));
			case 'Reset':
				return mk(
					$author$project$Main$reset(m));
			case 'Start':
				return mk(
					_Utils_update(
						m,
...
var $author$project$Main$buttonBar = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('button-bar')
			]),
		_List_fromArray(
			[
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$disabled(
								_Utils_eq(m.config.program, m.savedProgram))
							]),
						_List_fromArray(
							[
...
var $author$project$Main$envHtmls = function (e) {
	var go = function (bs) {
		if (!bs.b) {
			return _List_Nil;
		} else {
...
var $author$project$Main$viz = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						A2(
							...
							$author$project$Main$envHtmls(m.config.env)))
					])),
				A2(
				$elm$html$Html$button,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$id('clear-data'),
						$elm$html$Html$Attributes$disabled(
						$elm$core$List$isEmpty(m.config.stack) && $elm$core$Dict$isEmpty(m.config.env)),
						$elm$html$Html$Events$onClick($author$project$Main$ClearData)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('clear')
...
var $author$project$Main$window = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Events$preventDefaultOn, 'keydown', $author$project$Main$shortcuts)
			]),
		_List_fromArray(
			[
				A2(
